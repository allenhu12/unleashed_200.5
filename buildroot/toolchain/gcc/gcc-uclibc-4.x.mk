...
GCC_INSTALL=$(TARGET_DIR)
ifeq ($(BR2_GCC_USBTOOL),y)
GCC_INSTALL+=$(USB_TOOL_BUILD_DIR)
endif
...
GCC_WITH_HOST_GMP = --with-gmp=$(HOST_DIR)
GCC_WITH_HOST_MPFR = --with-mpfr=$(HOST_DIR)
...
ifeq ($(findstring x4.5.,x$(GCC_VERSION)),x4.5.)
GCC_WITH_HOST_MPC = --with-mpc=$(HOST_DIR)
...
ifeq (${BR2_GCOV},y)
	toolchain/patch-kernel.sh $(GCC_DIR) toolchain/gcc gcc-gcov-shared.patch
	toolchain/patch-kernel.sh $(GCC_DIR) toolchain/gcc gcc-dynamic-path-link.patch
endif
	#suppress the installation of libiberty.a. The version of libiberty.a provided by Binutils will be used instead
	sed -i 's/install_to_$$(INSTALL_DEST) //' $(GCC_DIR)/libiberty/Makefile.in
...
ifeq ($(BR2_GCC_SUPPORTS_FINEGRAINEDMTUNE),y)
	$(GCC_CONF_ENV) $(MAKE) -C $(GCC_BUILD_DIR1) -w all-gcc
else
	$(MAKE) -C $(GCC_BUILD_DIR1) -w all-gcc
endif
	touch $@

gcc_initial=$(GCC_BUILD_DIR1)/.installed
$(gcc_initial) $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc: $(GCC_BUILD_DIR1)/.compiled
	PATH=$(TARGET_PATH) $(MAKE) -C $(GCC_BUILD_DIR1) install-gcc
	touch $(gcc_initial)

gcc_initial: $(GCC_HOST_PREREQ) binutils $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc
...
#
#############################################################
GCC_BUILD_DIR2:=$(TOOL_BUILD_DIR)/gcc-$(GCC_VERSION)-intermediate
...
gcc_intermediate: uclibc-configured $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc
...
GCC_BUILD_DIR3:=$(TOOL_BUILD_DIR)/gcc-$(GCC_VERSION)-final
...
$(GCC_BUILD_DIR3)/.installed: $(GCC_BUILD_DIR3)/.compiled
	...
	if [ -d "$(STAGING_DIR)/lib64" ]; then \
		...
		rm -rf "$(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib64";\
	fi
	# Strip the host binaries
ifeq ($(GCC_STRIP_HOST_BINARIES),true)
	strip --strip-all -R .note -R .comment $(filter-out %-gccbug %-embedspu,$(wildcard $(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-*))
endif
	...
	mkdir -p $(TARGET_DIR)/lib $(TARGET_DIR)/sbin
	touch $@

$(STAMP_DIR)/gcc_libs_target_installed: $(GCC_BUILD_DIR3)/.installed
ifeq ($(BR2_GCC_SHARED_LIBGCC),y)
	# These go in /lib, so...
	rm -rf $(TARGET_DIR)/lib/libgcc_s*.so*
	-cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libgcc_s* \
		$(STAGING_DIR)/lib/
	-cp -dpf $(STAGING_DIR)/lib*/libgcc_s* \
		$(TARGET_DIR)/lib/
...
ifeq ($(BR2_GCC_SHARED_LIBGCC),y)
	...
	-cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libstdc++.so* \
		$(STAGING_DIR)/lib/
	-cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libstdc++.so* \
		$(TARGET_DIR)/lib/
	-$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/lib/libstdc++.so*
endif
endif
ifeq ($(BR2_INSTALL_LIBGCJ),y)
	cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libgcj.so* $(STAGING_DIR)/lib/
	cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libgcj.so* $(TARGET_DIR)/lib/
	mkdir -p $(STAGING_DIR)/lib/security
	mkdir -p $(TARGET_DIR)/lib/security
	cp -dpf $(STAGING_DIR)/lib/security/classpath.security \
		$(STAGING_DIR)/lib/security/
	cp -dpf $(STAGING_DIR)/lib/security/classpath.security \
		$(TARGET_DIR)/lib/security/
	-$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/lib/libgcj.so*
endif
ifeq ($(BR2_GCC_ENABLE_OPENMP),y)
	cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libgomp.so* $(STAGING_DIR)/lib/
	cp -dpf $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib*/libgomp.so* $(TARGET_DIR)/lib/
	-$(STRIPCMD) $(STRIP_STRIP_UNNEEDED) $(TARGET_DIR)/lib/libgomp.so*
endif
	mkdir -p $(@D)
	touch $@
...
$(TARGET_DIR)/lib/libgcc_s.so.1:
	# These are in /lib, so...
	for d in $(GCC_INSTALL); do \
		rm -rf $$d/usr/lib/libgcc_s*.so*; \
		$(STRIP) $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib/libgcc_s*.so.1; \
		sh -c "cp -a $(STAGING_DIR)/$(REAL_GNU_TARGET_NAME)/lib/libgcc_s* $$d/lib/; echo;";\
		if [ $(BR2_INSTALL_LIBSTDCPP)=y ]; then \
			sh -c "cp -a $(STAGING_DIR)/lib/libstdc++.so* $$d/lib/; echo;";\
		fi;\
		if [ $(BR2_INSTALL_LIBGCJ)=y ]; then \
			sh -c "cp -a $(STAGING_DIR)/lib/libgcj.so* $$d/lib/; echo;";\
			sh -c "cp -a $(STAGING_DIR)/lib/lib-org-w3c-dom.so* $$d/lib/; echo;";\
			sh -c "cp -a $(STAGING_DIR)/lib/lib-org-xml-sax.so* $$d/lib/; echo;";\
			sh -c "mkdir -p $$d/usr/lib/security; echo;";\
			sh -c "cp -a $(STAGING_DIR)/usr/lib/security/libgcj.security $$d/usr/lib/security/; echo;";\
			sh -c "cp -a $(STAGING_DIR)/usr/lib/security/classpath.security $$d/usr/lib/security/; echo;";\
		fi;\
	done;

#always install it again.
.PHONY: $(TARGET_DIR)/lib/libgcc_s.so.1

cross_compiler:=$(STAGING_DIR)/bin/$(REAL_GNU_TARGET_NAME)-gcc
cross_compiler gcc: gcc_intermediate \
	$(LIBFLOAT_TARGET) uclibc $(GCC_BUILD_DIR3)/.installed \
	$(STAMP_DIR)/gcc_libs_target_installed $(TARGET_DIR)/lib/libgcc_s.so.1 \
	$(GCC_TARGETS)
...
$(GCC_BUILD_DIR4)/.configured: $(GCC_BUILD_DIR4)/.prepared
	(cd $(GCC_BUILD_DIR4); rm -rf config.cache; \
		...
		--with-gmp=$(LIBGMP_DIR) \
		--with-gmp-include=$(LIBGMP_DIR) \
		--with-mpfr=$(BUILD_DIR)/mpfr-$(MPFR_VERSION)/ \
		--with-mpfr-include=$(BUILD_DIR)/mpfr-$(MPFR_VERSION)/src \
		--with-mpfr-lib=$(TARGET_DIR)/lib/ \
	)
	touch $@
...
ifeq ($(strip $(BR2_GCOV)),y)
    # gcov doesn't need all files
else
...
ifeq (${BR2_GCOV},y)
	# Generate .so and copy into target
	$(TARGET_CC) -shared -Wl,-soname,libgcov.so -o $(TARGET_DIR)/lib/libgcov.so -Wl,--whole-archive \
	$(GCC_BUILD_DIR4)/gcc/libgcov.a -Wl,--no-whole-archive
	if [ ! -d $(TARGET_DIR)/bin ]; then \
		mkdir -p $(TARGET_DIR)/bin; \
	fi
	install -m 755 $(GCC_BUILD_DIR4)/gcc/gcov $(TARGET_DIR)/bin
endif
gcc_target-clean:
